...
export async function generateMetadata() {
  const query = await getData(`{
    ...
    'data':*[_type=='guide'][0]{title,"summary":pt::text(summary),meta{title,description,keywords,"image":image.asset->url},cover{"image":image.asset->url, "vid":video.asset->playbackId, "ratio":video.asset->data.aspect_ratio}}
 }`)
 const {data, info} = query.data  
  return {
    ...
    openGraph: {
      images: data.meta.image?`${data.meta.image??info.meta.image}?auto=format&amp;w=500`: (data.cover.image?`${data.cover.image??info.meta.image}?auto=format&amp;w=500`:`${info.meta.image}?auto=format&amp;w=500`)
    }
  };
}
