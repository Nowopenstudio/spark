...
export async function Single({item,count}:any){
    ...
    const {project} = open
    console.log(project.settings.goals[0])
    ...
    <div className={` col-span-full lg:col-span-4 gridBox relative navTitle`}>
                  <div className="w-full p-[--sm]" >
                    <div className="relative bgBlur" style={{backgroundColor:`rgba(255, 255, 255, 0.1)`}}>
                      <div className="y-center absolute z-[10] left-[--xs] caption">${project.stats.balance.value/100}</div>
                      <div className="w-full h-[40px] mb-[--xs] growOn" style={{animationDelay:`${count*.75}s`,backgroundColor:`${item.color}`,width:`${(project.stats.balance.value/project.settings.goals[0].amount)*100}%`}}></div></div>
