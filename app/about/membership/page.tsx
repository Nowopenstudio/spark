...
export default async function Home({params}:{params:{slug:string}}) {
  const { data } = await getData(`*[_type=='membership'][0]{title,slug,'imageUrl': cover.asset->url, intro, content[]{desc,'image':image.asset->url,'vid':video.asset->{playbackId}},cover{"image":image.asset->url, "vid":video.asset->playbackId, "ratio":video.asset->data.aspect_ratio}}`)
  console.log(data[0])
  return (
